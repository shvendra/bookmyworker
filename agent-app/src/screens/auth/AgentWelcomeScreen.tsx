...
const FEATURES = [
  { icon: '🤝', title: 'Agent Network',    desc: 'Register workers, earn commission on every placement' },
  { icon: '💼', title: 'Find Work Nearby', desc: 'Browse job requirements in your district and state' },
  { icon: '📊', title: 'Track Earnings',   desc: 'Real-time wallet, commission & attendance tracking' },
  { icon: '✅', title: 'Get Verified',     desc: 'Verified badge boosts trust and job opportunities' },
];
        ...
        <SafeAreaView>
          ...
          >
            ...
            <AppText variant="body" color="rgba(255,255,255,0.78)" style={styles.heroSubtitle}>
              India's platform for agents, job seekers{'\n'}& skilled workers to grow together
            </AppText>
            <View style={styles.proofPill}>
              <View style={styles.proofDots}>
                {['#10B981', '#F59E0B', '#3B82F6'].map((c, i) => (
                  <View
                    key={c}
                    style={[styles.proofDot, { backgroundColor: c, marginLeft: i > 0 ? -6 : 0, borderColor: primary }]}
                  />
                ))}
              </View>
              <AppText variant="micro" color="rgba(255,255,255,0.9)" style={styles.proofText}>
                5 Lakh+ workers earning daily
              </AppText>
            </View>
          </Animated.View>
        </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  proofDot: { width: 20, height: 20, borderRadius: 10, borderWidth: 2 },
