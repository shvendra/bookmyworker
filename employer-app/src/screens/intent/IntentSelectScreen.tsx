...
const BRAND = '#1037A4';
// The Worker/Agent app on the Play Store — where "I'm looking for work" users belong.
const WORKER_PLAY_URL = 'https://play.google.com/store/apps/details?id=com.app.myworker';

type Props = NativeStackScreenProps<EmployerStackParamList, 'IntentSelect'>;

export const IntentSelectScreen = ({ navigation }: Props): React.JSX.Element => {
  const insets = useSafeAreaInsets();
  // Intent keys live in the default `translation` namespace (shared by both apps).
  ...
  // Correct choice for the Employer app → continue the normal flow, unchanged.
  const chooseHire = () => navigation.replace('Welcome');

  // Wrong choice (they want work) → send them to the Worker app on the Play Store.
  const chooseWork = () => {
    if (redirecting) return;
    setRedirecting(true);
    Linking.openURL(WORKER_PLAY_URL).catch(() => setRedirecting(false));
  };
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        ...
        <AppText style={styles.heroSub}>{t('intentSubtitleEmployer')}</AppText>
      ...
      <Animated.View style={[styles.body, { opacity: fade, transform: [{ translateY: slide }] }]}>
        <Card icon="🧑‍🔧" title={t('intentHireWorkers')} desc={t('intentHireWorkersDesc')} onPress={chooseHire} accent />
        <Card icon="🔍" title={t('intentFindWork')} desc={t('intentFindWorkDesc')} onPress={chooseWork} />
        {redirecting && <AppText style={styles.redirectNote}>{t('intentRedirectWorker')}</AppText>}
      </Animated.View>
    </View>
  );
};
